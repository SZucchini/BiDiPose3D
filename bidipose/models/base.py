...
class BaseModel(nn.Module):
    ...
    def forward(
        ...
        Args:
            ...
            t (torch.Tensor | None): Diffusion time step (B,).

        Returns:
            pred_pose (torch.Tensor): Predicted 2D poses from two-views (B, T, J, 3*2).
            pred_quat (torch.Tensor): Predicted quaternions for cam1 to cam2 (B, 4).
            pred_trans (torch.Tensor): Predicted translations for cam1 to cam2 (B, 3).
