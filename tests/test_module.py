...
def test_forward(module):
    ...
    quat = torch.randn(2, 3, 4, 4)
    trans = torch.randn(2, 3, 4, 3)
...
def test_training_step_logs(module):
    ...
    assert module.log.call_count == 4
...
def test_process_data_for_logging(module):
    x = [torch.randn(2, 3, 4, 5), torch.randn(2, 3, 4, 5)]
    x_gt = [torch.randn(2, 3, 4, 5), torch.randn(2, 3, 4, 5)]
...
def test_log_animation(mock_vis_pose3d, mock_vis_pose2d, module, tmp_path):
    ...
    module.log_videos = MagicMock()
    x = [torch.randn(1, 3, 4, 5)]
    quat = [torch.randn(1, 3, 4, 4)]
    trans = [torch.randn(1, 3, 4, 3)]
    x_gt = [torch.randn(1, 3, 4, 5)]
    quat_gt = [torch.randn(1, 3, 4, 4)]
    trans_gt = [torch.randn(1, 3, 4, 3)]
    module._log_animation(
        key="test",
        x=x, quat=quat, trans=trans,
        x_gt=x_gt, quat_gt=quat_gt, trans_gt=trans_gt
    )
    assert module.log_videos.call_count == 2
