...
def test_get_camera_mask():
    ...
    assert torch.all(mask[:, :, :, :3] == 1), "Camera mask values mismatch"
    assert torch.all(mask[:, :, :, 3:] == 0), "Camera mask values mismatch for non-indexed camera"

    camera_index = 1
    mask = get_camera_mask(x, camera_index)
    assert mask.shape == x.shape, "Camera mask shape mismatch for second camera"
    assert torch.all(mask[:, :, :, 3:] == 1), "Camera mask values mismatch for second camera"
    assert torch.all(mask[:, :, :, :3] == 0), "Camera mask values mismatch for non-indexed camera"
